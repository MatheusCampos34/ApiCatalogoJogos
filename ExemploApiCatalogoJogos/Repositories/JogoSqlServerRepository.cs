...
namespace ExemploApiCatalogoJogos.Repositories
{
    public class JogoSqlServerRepository : IJogoRepository
    {
        ...
        public async Task<List<Jogo>> Obter(int pagina, int quantidade)
        {
            ...
            while (sqlDataReader.Read())
            {
                jogos.Add(new Jogo
                {
                    ...
                    Console = (string)sqlDataReader["Console"],
                    Ano = (double)sqlDataReader["Ano"]
                });
            }
        ...
        {
            var comando = $"insert Jogos (Id, Nome, Produtora, Preco, Console, Ano) values ('{jogo.Id}', '{jogo.Nome}', '{jogo.Produtora}', {jogo.Preco.ToString().Replace(",", ".")}, '{jogo.Console}', {jogo.Ano.ToString().Replace(",", ".")})";
        ...
        {
            var comando = $"update Jogos set Nome = '{jogo.Nome}', Produtora = '{jogo.Produtora}', Preco = {jogo.Preco.ToString().Replace(",", ".")}, Console = '{jogo.Console}', Ano = {jogo.Ano.ToString().Replace(",", ".")} where Id = '{jogo.Id}'";
